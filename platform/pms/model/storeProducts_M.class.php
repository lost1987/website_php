<?php
    ...
    class StoreProducts_M extends XfModel
    {
        ...
        {
            $sql = "SELECT * FROM xf_platform_store_products WHERE id = ?";
        ...
        {
            return $this->db->save( $fields , 'xf_platform_store_products' );
        }

        function del( $id )
        {
            $sql = "DELETE FROM xf_store_productorder WHERE product_id = ? ;DELETE FROM xf_platform_store_products  WHERE id = ?";
        ...
        {
            return $this->db->update( $fields , 'xf_platform_store_products' , " WHERE id=$id" );
        }
        ...
        function lists( $start = null , $count = null )
        {
            ...
            $sql = "SELECT * FROM xf_platform_store_products  {$this->_condition}  $limit";
            $this->db->execute( $sql );
            $this->_condition = '';
        ...
        function num_rows()
        {
            $sql = "SELECT COUNT(*) as num FROM  xf_platform_store_products  {$this->_condition}";
            $this->db->execute( $sql );
            $this->_condition = '';
