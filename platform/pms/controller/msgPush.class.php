    ...
    class MsgPush extends AdminController
    {
        ...
        function goPush()
        {
            ...
            switch ($post['device_type']) {
                case 0:      AndroidPusher::instance()->send( $post['title'] , $post['content'] , $post['msg_type'] , $this->config->pms['notification_product_mode']  );
                                //读取ios设备
                                ...
                                IosPusher::instance()->send( $post['content'] , $alias ,$this->config->pms['notification_product_mode']);
                    break;
                case 1:
                    $result = AndroidPusher::instance()->send( $post['title'] , $post['content'] , $post['msg_type'] , $this->config->pms['notification_product_mode']  );
                    break;
                case 2:
                        //读取ios设备
                        $devices = ImeiModel::instance()->lists( ImeiModel::IOS );
                    ...
                    $result = IosPusher::instance()->send( $post['content'] , $alias ,$this->config->pms['notification_product_mode']);
                    break;
                default :
...
//            $query_string = http_build_query( $search_params );
            ...
            $this->init_navigator();
            $list = MsgPushTask_M::instance()->lists($start,$count);
...
//            }
        ...
        function taskSave(){
            ...
            if(!$this->db->save($post,'pms_msg_push_task'))
