<?php
    ...
    class AdminController extends Base
    {
        ...
        function csrf_token_validation( $update_after_validation = true , $_token = null )
        {
            ...
            $csrf_token_name = $config->pms['csrf']['token_name'];
            if ( !empty( $_token ) )
                $token = $_token;
            else
        ...
        protected function init_navigator()
        {
            ...
            $modules = BaseModule::instance()->lists();
            ...
            //读取服务器分类
            $this->output_data['servers'] = BaseServers::instance()->lists();
            $serverJson = Cookie::instance()->userdata( 'server' );
            if ( !empty( $serverJson ) ) {
                $server = Encoder::instance()->decode( $serverJson );
                $this->output_data['currentServer'] = $server['game_name'];
            } else
