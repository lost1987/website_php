    ...
    class Omni
    {
        ...
        private function __construct()
        {
            ...
            switch (RUN_MODE) {
                ...
                case 'command'://命令号模式 例如 API接口
                    $this->command_init();
                    break;

                case 'task'://任务模式 例如 统计进程
                    $this->task_init();
                    break;

                default:  //默认:web fpm模式
                    $this->fpm_init();
            }
        }
        ...
        private function command_init()
        {
            ...
            /*运行pathinfo 解析**/
            $this->_startup = \core\Command::instance();
        }

        private function task_init(){
            /*设置编码**/
            header( "content-type:text/html;charset=utf8" );

            /*运行pathinfo 解析**/
            $this->_startup = \core\Task::instance();
        }
        ...
        function run( $dir = null )
        {
            ...
            $init_redis = $_config['init_redis'];
            if ( $init_redis ) {
                $r = new \core\Redis( $this->config->common['redis']['host'] , $this->config->common['redis']['port'] );
                $this->redis = $r->getResource();
                $this->redis->select( $this->config->common['redis']['idx'] );
            }
