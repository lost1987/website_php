<?php
    ...
    class DB
    {
         ...
         * 根据配置文件来初始化
         * @param $dbname string
         * @param $persist bool 是否持久连接
         * @throws \Exception
         */
        public function init_db_from_config( $dbname ,$persist = false)
        {
            ...
            $this->_resouce = new \PDO( "mysql:host=$this->_host;dbname=$dbname;charset=$this->_charset" , $this->_user , $this->_passwd ,$options);
         ...
         * @throws \Exception
         */
        public function init_db( $server ,$persist = false)
        {
            ...
            $this->_charset = 'utf8';
            $this->_dbname = $dbname = $server['server_db_name'];
        ...
        /**
         * 设置当前会话连接等待超时时间(运行脚本任务时 非常重要)
         * @param $timeOut 秒
         * @return bool
         */
        function setSessionWaitTimeOut($timeOut){
            $stmt = $this->_resouce->prepare("set session wait_timeout=$timeOut,interactive_timeout=10000,net_read_timeout=10000");
            return $stmt ->execute();
        }
