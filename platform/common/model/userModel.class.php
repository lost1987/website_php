<?php
    ...
    class UserModel extends XfModel
    {
         ...
         * @return mixed
         */
        function getUserByLoginName( $username ,$fields = '*')
        {
            if(is_array($fields))
                $fields = implode(',',$fields);

            $sql = "SELECT $fields  FROM xf_user WHERE login_name = ?";
        ...
        function getUserByUserNumber( $user_number,$fields = '*' )
        {
            if(is_array($fields))
                $fields = implode(',',$fields);

            $sql = "SELECT $fields FROM xf_user WHERE user_number = ?";
        ...
        function getUserByInviteCode( $inviteCode ,$fields='*')
        {
            if(is_array($fields))
                $fields = implode(',',$fields);

            $sql = "SELECT $fields FROM xf_user WHERE invite_code = ?";
        ...
        function getUserByMail( $mail,$fields='*' )
        {
            if(is_array($fields))
                $fields = implode(',',$fields);

            $sql = "SELECT $fields  FROM xf_user WHERE email = ?";
            $this->db->execute( $sql , array( $mail ) );

            return $this->db->fetch();
        }

        function getUserByIDCard( $idCard,$fields='*' )
        {
            if(is_array($fields))
                $fields = implode(',',$fields);

            $sql = "SELECT $fields FROM xf_user WHERE id_card = ?";
            $this->db->execute( $sql , array( $idCard ) );

            return $this->db->fetch();
        }

        function getUserByMobile( $mobile ,$fields = '*')
        {
            if(is_array($fields))
                $fields = implode(',',$fields);

            $sql = "SELECT $fields FROM xf_user WHERE mobile = ?";
         ...
         * @return mixed
         */
        function getUserByUid( $uid ,$fields = '*')
        {
            if(is_array($fields))
                $fields = implode(',',$fields);

            $sql = "SELECT $fields FROM xf_user WHERE user_id = ?";
            $this->db->execute( $sql , array( $uid ) );
