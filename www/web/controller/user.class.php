<?php
    ...
    class User extends Controller
    {
        ...
        function login()
        {
            ...
            if ( !empty( $username )
                ...
                if ( UserUtil::instance()->is_password_valid( $password , $user['password'] , $user['user_number'] ) ) {
                    ...
                    if($login_type == 'pt')
                        $redirect->addGetParam( 'code' , $errcode )->forward(PLATFORM_HOST);
                ...
                if ( $gamelogin )
                    $redirect->addGetParam( 'code' , $errcode )->forward( '/game' );
                else
                    $redirect->addGetParam( 'code' , $errcode )->forward();
            }
            //用户名或密码为空
        ...
        private function loginSync( $user , $xfDB , $admDB )
        {
            \uad\libs\LoginSync::instance()->loginSuccess( $user , $xfDB , $admDB );
        }
        ...
        function register( $post_fields = null )
        {
            ...
            try {
                ...
                if ( !empty( $invite_code ) ) {
                    if ( !UserRelationShip::instance( $admDb , $this->db )->bindParent( $uid , $invite_code ) )
                        throw new \Exception( Error::INVALID_INVITE_CODE );
                }
                ...
                DataUtil::instance()->doAfterRegister( $platform , $uid , 1 , 1 );
                $this->loginSync( $fields1 , $this->db , $admDb );
